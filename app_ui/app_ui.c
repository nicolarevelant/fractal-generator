...
static const char *authors[] = {"Nicola Revelant <nicolarevelant@outlook.com>",
                                NULL};
...
static gboolean on_window_key_pressed(GtkEventControllerKey *, guint key, guint,
...
int start_ui(int argc, char **argv) {
    AdwApplication *app =
        adw_application_new(APPLICATION_ID, G_APPLICATION_DEFAULT_FLAGS);
...
static void on_app_activate(GtkApplication *app) {
    ...
    g_signal_connect_swapped(aboutBtn, "clicked", G_CALLBACK(show_about_dialog),
                             window);
    ...
    if (!tmpWidget) {
        debug_printerr(
            " [EE] Cannot create main layout: Home layout is NULL\n");
        g_object_unref(builder);
    ...
    if (!tmpWidget) {
        debug_printerr(
            " [EE] Cannot create main layout: Settings layout is NULL\n");
        g_object_unref(builder);
        gtk_window_destroy(window);
        return;
    }
    ...
    g_signal_connect(eventController, "key-pressed",
                     G_CALLBACK(on_window_key_pressed), NULL);
...
static void show_about_dialog(GtkWindow *window) {
    adw_show_about_dialog(GTK_WIDGET(window), "application-name", PROJECT_NAME,
                          "application-icon", ICON_NAME, "developer-name",
                          "Nicola Revelant", "version", PROJECT_VERSION,
                          "copyright", PROJECT_COPYRIGHT, "comments",
                          PROJECT_DESCRIPTION, "license", PROJECT_LICENSE,
                          "developers", authors, "designers", designers, NULL);
}

/*        --- EVENTS ---               */

void on_window_destroy() { debug_printerr(" [DD] On destroy\n"); }

gboolean on_window_key_pressed(GtkEventControllerKey *, guint key, guint,
                               GdkModifierType state, gpointer) {
    ...
    double translate_step =
        gtk_spin_button_get_value(GTK_SPIN_BUTTON(translateStep_spinBtn));
    double zoom_step =
        gtk_spin_button_get_value(GTK_SPIN_BUTTON(zoomStep_spinBtn));
    ...
    if (translate_x || translate_y) {
        if (mb_tmp_config.use_julia) {
            if (state & GDK_CONTROL_MASK) {
                mb_tmp_config.x += translate_x / ((state & GDK_ALT_MASK)
                                                      ? mb_tmp_config.julia_zoom
                                                      : mb_tmp_config.zoom);
                mb_tmp_config.y += translate_y / ((state & GDK_ALT_MASK)
                                                      ? mb_tmp_config.julia_zoom
                                                      : mb_tmp_config.zoom);
            } else {
