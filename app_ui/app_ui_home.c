...
static void on_photo_dialog_response(GObject *fileDialog, GAsyncResult *res,
                                     gpointer data);

static void btn_save_video_clicked(GtkWidget *btn);
static void on_video_dialog_response(GObject *fileDialog, GAsyncResult *res,
                                     gpointer data);
...
static fractal_config_t get_final_config(fractal_config_t *old, int width,
                                         int height);
...
GtkWidget *create_home_layout() {
    ...
    GError *error = NULL;
    if (!gtk_builder_add_from_file(builder, DATADIR "/ui/app_ui_home.ui",
                                   &error)) {
...
void btn_save_photo_clicked(GtkWidget *btn) {
    ...
    guint size_selected =
        gtk_drop_down_get_selected(GTK_DROP_DOWN(frameSizeDropDown));
    if (size_selected == GTK_INVALID_LIST_POSITION)
        return;
    ...
    g_object_set(fileDialog, "title", _("Photo"), "initial-name",
                 mb_photo_filename ? mb_photo_filename : MB_DEFAULT_PHOTO_NAME,
                 NULL);

    gtk_file_dialog_save(fileDialog, GTK_WINDOW(root), NULL,
                         on_photo_dialog_response, NULL);
}

void on_photo_dialog_response(GObject *fileDialog, GAsyncResult *res,
                              __attribute__((unused)) gpointer data) {
    GFile *file =
        gtk_file_dialog_save_finish(GTK_FILE_DIALOG(fileDialog), res, NULL);
    if (file) {
        ...
        fractal_config_t new_config =
            get_final_config(&mb_tmp_config, mb_width, mb_height);
        fractal_begin_photo(&new_config, mb_photo_filename,
                            mb_on_photo_progress, image_on_save);
    } else {
...
void btn_save_video_clicked(GtkWidget *btn) {
    ...
    g_object_set(fileDialog, "title", _("Video"), "initial-name",
                 mb_video_filename ? mb_video_filename : MB_DEFAULT_VIDEO_NAME,
                 NULL);

    gtk_file_dialog_save(fileDialog, GTK_WINDOW(root), NULL,
                         on_video_dialog_response, NULL);
}

void on_video_dialog_response(GObject *fileDialog, GAsyncResult *res,
                              __attribute__((unused)) gpointer data) {
    GFile *file =
        gtk_file_dialog_save_finish(GTK_FILE_DIALOG(fileDialog), res, NULL);
    if (file) {
        ...
        mb_video_config_t video_config = {
            gtk_spin_button_get_value(
                GTK_SPIN_BUTTON(video_init_zoom)), // initial zoom value
            gtk_spin_button_get_value(
                GTK_SPIN_BUTTON(video_zoom_speed)), // zoom step
            mb_framerate};

        fractal_begin_video(&new_config, &video_config, mb_video_filename,
                            mb_on_video_progress, video_on_save);
    } else {
...
static void btn_stop_video_clicked() { mb_video_stop(); }
...
gboolean ui_on_update(UpdateData *data) {
    ...
    case IDLE:
        ...
        break;
    case PHOTO_PROGRESS:
        snprintf(stateProgressStr, STATE_PROGRESS_SIZE, _("Progress: %.2f %%"),
                 progress * 100.0f);
        gtk_label_set_text(statusLabel, stateProgressStr);
        gtk_progress_bar_set_fraction(progressBar, progress);
        break;
    case VIDEO_PROGRESS:
        snprintf(stateProgressStr, STATE_PROGRESS_SIZE,
                 _("Progress: %02d:%02d.%02d"),
